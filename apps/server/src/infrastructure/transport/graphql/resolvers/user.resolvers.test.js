const { createUserHelper, updateUserSummaryHelper, getUserByIdHelper, listUsersHelper } = require('../../../../utils/tests/user.resolver.helpers');
...
describe('User Resolvers', () => {
  ...
  it('should return an user with updated summary when UpdateUserSummary is called', async () => {
    ...
    const { data: { updateUserSummary} } = await testServer.executeOperation(updateUserSummaryHelper(createUser.id, 'any_summary'))
    expect(updateUserSummary.summary).toBe('any_summary')
  })

  it('should return an user when getUserById is called', async () => {
    const { data: { createUser} } = await testServer.executeOperation(createUserHelper())
    const { data: { getUserById }} = await testServer.executeOperation(getUserByIdHelper(createUser.id))
